...
class APIRequests:  
   ...
   def ReturnFundamentalData(tag, ReceiveType):
    ...
    url = 'https://www.alphavantage.co/query?function='+ReceiveType+'&symbol='+tag+'&apikey=DEMOKEY'
    r = requests.get(url)
    Data = r.json()
    return Data
   ...
   def SearchEndpoint(search_value):
    url = 'https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords='+search_value+'&apikey=DEMOKEY'
    r = requests.get(url)
    Data = r.json()
    return Data 

   def ReceiveCompanyOverview(tag):
    url = 'https://www.alphavantage.co/query?function=OVERVIEW&symbol='+tag+'&apikey=DEMOKEY'
    r = requests.get(url)
    Data = r.json()
    print(Data)
    return Data
   ...
   def ReceivePriceInformation(frequency, tag):
    url = 'https://www.alphavantage.co/query?function=TIME_SERIES_'+frequency+'_ADJUSTED&symbol='+tag+'&apikey=DEMOKEY'
    r = requests.get(url)
    Data = r.json()
    return Data
